...
from .bound_op import LowerBound
...
class _EntropyCoder:
    ...
    def __init__(self, method):
        ...
        if method == "ans":
            ...
            decoder = ans.Decoder(encoded)
        elif method == "rangecoder":
            import range_coder  # pylint: disable=E0401
            from tempfile import mkstemp
            filepath = mkstemp()[1]
            encoder = range_coder.RangeEncoder(filepath)
            decoder = range_coder.RangeDecoder(filepath)
...
class EntropyModel(nn.Module):
    ...
    def quantize(
        ...
        outputs = torch.round(outputs) # rounded to the closest integer
        ...
        outputs = outputs.int() # 已经round了为什么还要int()？是为了从float转为int
        return outputs
    ...
    def compress(self, inputs, indexes, means=None):
        ...
        """
        symbols = self.quantize(inputs, "symbols", means)  # round(input-means)
        ...
        strings = []
        for i in range(symbols.size(0)): # 按Batch循环
            rv = self.entropy_coder.encode_with_indexes( # 调用C++接口进行熵编码
    ...
    def decompress(self, strings, indexes, means=None):
        ...
        outputs = self.dequantize(outputs, means) # 变成mean同类型的 或 变成float型变量
        return outputs
    ...
    *tensorflow/compression*. See the original paper and the `tensorflow
    ...
    def __init__(
        ...
        **kwargs,
        ...
        scale = self.init_scale ** (1 / (len(self.filters) + 1)) # 10 ** (1/6 + 1)
        channels = self.channels

        for i in range(len(self.filters) + 1): # 创建了一些EntropyBottleneck的参数矩阵
        ...
        target = np.log(2 / self.tail_mass - 1) # target 又是什么？？？
    ...
    def _logits_cumulative(self, inputs, stop_gradient):
        ...
        logits = inputs
        for i in range(len(self.filters) + 1): # 通过EntropyBottleneck，即与EntropyBottleneck的参数矩阵相乘
    ...
    def forward(self, x):
        ...
        shape = x.size()
        values = x.reshape(x.size(0), 1, -1) # 转为(C,1,H*W*B)
    ...
    def _build_indexes(size):
        N, C, H, W = size
        indexes = torch.arange(C).view(1, -1, 1, 1) # [0,1,2,...,C-1] (C) -> (1,C,1,1)
        indexes = indexes.int() # self.to(torch.int32)
        return indexes.repeat(N, 1, H, W) # (1,C,1,1) -> (N,C,H,W)
...
class GaussianConditional(EntropyModel):
    ...
    *tensorflow/compression*. See the `tensorflow documentation
    ...
    def __init__(self, scale_table, *args, scale_bound=0.11, tail_mass=1e-9, **kwargs):
        ...
        if not isinstance(scale_table, (type(None), list, tuple)): # scale table是一个列表，包含了（默认64个）预先计算好的scale
        ...
        if scale_table and ( # scale_table应该从小到大排序且均大于0
        ...
        if scale_bound is None and scale_table:
            self.lower_bound_scale = LowerBound(self.scale_table[0]) # 定义一个scale的下界函数，scale下界为scale_table最小的元素，这个下界函数的梯度自定义了
                                                                     # todo:什么情况走这里？（是加载模型后走这里吗？）
        elif scale_bound > 0:
            self.lower_bound_scale = LowerBound(scale_bound) # 一般初始化我们走这里，scale下界为0.11
        else:
        ...
        ) # 没有scale_table,则初始值为torch.Tensor()
    ...
    def _standardized_quantile(quantile):
        return scipy.stats.norm.ppf(quantile)  # 返回 当标准正态分布(μ=0，σ=1)的概率累计函数(cdf)值为quantile的时候,x所取的值 ppf是cdf的反函数
    ...
    def update_scale_table(self, scale_table, force=False):
        ...
        self.scale_table = self._prepare_scale_table(scale_table).to(device) # self.scale_table 包含64个变量[0.11~256]
        self.update()
        return True

    def update(self):
        multiplier = -self._standardized_quantile(self.tail_mass / 2) # multiplier是最小概率质量/2对应的那个随机变量的值
                                                                      # tail_mass意思应该是最小概率质量。为啥tail_mass还要除2？为啥要加个负号？
        pmf_center = torch.ceil(self.scale_table * multiplier).int() # 像是计算每个scale的multiplier，跟概率质量函数的中心有什么关系？
                                                                     # ceil向上取整，为啥要取整？为啥变成概率质量函数的中心了？？？
        pmf_length = 2 * pmf_center + 1
        max_length = torch.max(pmf_length).item()

        device = pmf_center.device
        samples = torch.abs(
            torch.arange(max_length, device=device).int() - pmf_center[:, None]  # [0,max_length-1] - pmf_center
        ) # sample像是每个scale对应的采样点？
        samples_scale = self.scale_table.unsqueeze(1) # 增加维度1
        ...
        pmf = upper - lower # 计算每个采样离散点的概率

        tail_mass = 2 * lower[:, :1]

        quantized_cdf = torch.Tensor(len(pmf_length), max_length + 2)
        quantized_cdf = self._pmf_to_cdf(pmf, tail_mass, pmf_length, max_length) # 调用C++接口根据pmf计算cdf
        self._quantized_cdf = quantized_cdf
    ...
    def build_indexes(self, scales):
        # scales尺寸与y一样
        scales = self.lower_bound_scale(scales) #判断是否低于scale下界,若低于下界值,就把这个值置为下界值
        indexes = scales.new_full(scales.size(), len(self.scale_table) - 1).int() # index为 全部填充为 值为len(self.scale_table)-1;size与scales同样的tensor
        for s in self.scale_table[:-1]: # s从scale_table第一个元素 遍历到 倒数第二个元素
