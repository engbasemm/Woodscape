...
from .functional import rgb2ycbcr, ycbcr2rgb
...
class HM(Codec):
    ...
    def _run(self, img, quality, return_rec=False, return_metrics=True):
        ...
        out_filepath = os.path.splitext(yuv_path)[0] + ".bin"
        bitdepth = 8
        arr = np.concatenate((img[0], img[1], img[2]), axis=2)
        #arr = arr.transpose((1, 2, 0))  # color channel first
        #plt.imshow(arr, interpolation='nearest')
        #plt.show()

        #arr = img2.transpose((2, 0, 1))  # color channel first

        if not self.rgb:
            # convert rgb content to YCbCr
            #rgb = torch.from_numpy(arr.copy()).float() / (2 ** bitdepth - 1)
            arr = torch.tensor(arr)
            arr = np.clip(rgb2ycbcr(arr).numpy(), 0, 1)
            arr = (arr * (2 ** bitdepth - 1)).astype(np.uint8)
        ...
        ''''''''' 
        if not self.rgb:
            arr = rgb_to_yuv420(img2)
            #arr = arr.numpy()

        #torchvision.utils.save_image(arr.data, yuv_path,normalize=True)
        arr = arr[0].numpy()
        #arr = arr.transpose((1,0,2, 3))  # color channel first
        '''
        with open(yuv_path, "wb") as f:
            f.write(arr.tobytes())
        ...
        if not self.rgb:
            arr = ycbcr2rgb(torch.from_numpy(arr.copy())).numpy()
            rec_arr = ycbcr2rgb(torch.from_numpy(rec_arr.copy())).numpy()
#            arr = rec_arr.transpose((1, 2, 0))  # color channel first
#            plt.imshow(arr, interpolation='nearest')
#            plt.show()
        ...
        #    )
        #    return out, rec
        rec_arr = np.array_split(rec_arr,3,axis=2)  # color channel first
        rec_arr = np.reshape(rec_arr, (3, 3, 288, 544))
        #plt.imshow(arr1[0].transpose(1, 2, 0), interpolation='nearest')
        #plt.show()

        return out ,torch.tensor(rec_arr)
