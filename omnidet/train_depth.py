"""
Depth Estimation training for OmniDet.
...
class DepthModelBase(TrainUtils):
    def __init__(self, args):
        super().__init__(args)

        # --- INIT MODELS ---
        self.models["encoder"] = ResnetEncoder(num_layers=self.args.network_layers, pretrained=True).to(self.device)
        self.models["norm"] = DepthDecoder(self.models["encoder"].num_ch_enc,range(args.num_scales)).to(self.device)
    ...
    def depth_train(self):
        """Trainer function for depth and depth prediction on fisheye images"""
...
class DepthModel(DepthModelBase):
    def __init__(self, args):
        super().__init__(args)
