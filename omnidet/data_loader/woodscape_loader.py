...
class WoodScapeRawDataset(data.Dataset):
    ...
    def __init__(self, data_path=None, path_file=None, is_train=False, config=None):
        ...
        #self.resize  = transforms.Compose([transforms.Resize([self.network_input_height, self.network_input_width],interpolation=transforms.InterpolationMode.BICUBIC),
        #                                #transforms.ToTensor() ,
        #                                #transforms.Lambda(lambda x: x.repeat(3, 1, 1)),
        #                                # gray -> GRB 3 channel (lambda function)
        #                                transforms.Normalize(mean=[0.5, 0.5, 0.5],
        #                                                   std=[0.5, 0.5, 0.5])])  # for grayscale images
